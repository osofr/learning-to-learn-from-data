...
function trainepochs(o,nets,modelP,lossNormalizer,atype=Array{Float32})
    ...
    if (o[:maxRiskInit]==1)
        ...
        println((:epoch,0,:trueMaxRisk,maxRisk[1],:hardestγ,hardestγ));
        if o[:Rgrid]==1
    ...
    for epoch = 1:o[:nepochs]
        ...
        if (epoch%o[:maxRiskEvery]==0)
            ...
            println((:epoch,epoch,:trueMaxRisk,maxRisk[div(epoch,o[:maxRiskEvery])+o[:maxRiskInit]],:hardestγ,hardestγ));
            if o[:Rgrid]==1
                println("computing risk surface with grid size $(o[:Rgridsize])")
...
function Piᵏloss(mPiᵏ,mPiᵣ,mTᵏ,noise,z1n,modelP,lossNormalizer)
    ...
    γsamples = predictPiᵏ(mPiᵏ,noise,modelP) # γsamples = mPiᵏ(noise)
    ...
    # println((:λpenalty,getval(λpenalty)))
