...
## **********************************************************************
## 3. Deep interrogatation of the Confidence Region estimator (run on GPU).
...
##    Evaluating for σ parameters ≧ 1.0.
## **********************************************************************
...
ngrid = 500 ## number of random γ for random grid search
...
lb = maximinNN1.lb
ub = maximinNN1.ub
gridμ = linspace(lb(modelP)[1], ub(modelP)[1], o[:ngrid])
gridσ = linspace(lb(modelP)[2], ub(modelP)[2], o[:ngrid])
...
#
# println("----------------------------------------")
# println("FINISHED GRID INTERROGATION #1")
# println("----------------------------------------")
...
# JLD.save(joinpath(writedir, "deep_CI_risks_grid1_142.1K.jld"),
...
# writedir = "./saved_models_offsetsLU/risks/"
# res = JLD.load(joinpath(writedir, "deep_CI_risks_grid1_142.1K.jld"))

## -----------------------------------------------------------------------------
## GRID INTERROGATION #2 (for best model, max and mean risks based on new batch size and new # of random γ)
## -----------------------------------------------------------------------------
srand(54321);
max_tIerr = ones(Float64, length(ηvals));
maxRγD = Any[]
...
println("----------------------------------------")
println("FINISHED GRID INTERROGATION #2")
println("----------------------------------------")
println("max_tIerr: "); println(max_tIerr)
println("----------------------------------------")
println("maxRγD[1][:typeIerr]: "); println(maxRγD[1][:typeIerr])
...
writedir = "./saved_models_offsetsLU/risks/"
JLD.save(joinpath(writedir, "deep_CI_risks_grid2_142.1K.jld"),
  "rEpoch", rEpoch, "etas", etas, "max_tIerr", max_tIerr, "xgridgamma", xgridgamma,
  "Risks_typeIerr", Risks_typeIerr, "Risks_len", Risks_len, "Risks_lenScaled",
  Risks_lenScaled, "Risks_cent", Risks_cent, "maxRgammaD", maxRgammaD)
